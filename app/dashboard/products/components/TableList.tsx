  ...
  const onDeleteProduct = async (id: string) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.success(
          data.message ||
            (isBn ? "পণ্য ডিলিট成功了!" : "Product deleted successfully!"),
        );
        router.refresh();
        ...
        toast.error(
          data.error ||
            (isBn ? "পণ্য ডিলিট করতে ব্যর্থ" : "Failed to delete product"),
        );
      }
    } catch (error) {
      ...
      <div className="p-6 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-gray-50/30">
        <div className="flex items-center gap-2">
          <span className="text-sm font-medium text-gray-600">
            {isBn ? "প্রতি পৃষ্ঠায় দেখান:" : "Show items:"}
          </span>
          <select
            value={itemsPerPage}
        ...
        <table className="w-full text-left border-collapse text-sm">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-100 text-gray-600 font-semibold">
              <th className="p-4">
                {isBn ? "পণ্যের ছবি ও নাম" : "Product & Info"}
              </th>
            ...
            {paginatedProducts.map((product) => (
              ...
              >
                ...
                <td className="p-4 text-gray-600">
                  {isBn
                    ? product.brand.brandNameBn || product.brandName
                    : product.brandName}
                </td>
                <td className="p-4">
                ...
                <td className="p-4">
                  ...
                  <div className="text-xs text-gray-500 mt-1">
                    {isBn ? "স্টক:" : "Stock:"}
                    <span
                      className={`ml-1 font-medium ${product.stock <= 10 ? "text-red-500" : "text-gray-700"}`}
                    >
                      {product.stock}
                    </span>
                  </div>
